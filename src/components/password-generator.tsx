...
export function PasswordGenerator() {
      ...
      switch (generationType) {
        ...
        case '6-digit':
          ...
          if (digitsOnly6.length < 6) {
            result = 'Error: Not enough digits';
  ...
  const handleCopy = () => {
    if (generatedPassword && !generatedPassword.startsWith('Error:')) {
      ...
      {(generatedPassword || isLoading) && (
        <CardFooter className="flex flex-col">
          ...
          {isLoading ? (
            ...
            <div className="relative w-full rounded-lg border bg-secondary p-4 animate-in fade-in duration-500">
                <p className={`font-code text-lg tracking-wider break-all ${generatedPassword.startsWith('Error:') ? 'text-destructive' : 'text-secondary-foreground'}`}>{generatedPassword}</p>

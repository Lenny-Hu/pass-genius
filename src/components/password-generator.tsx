              ...
              <RadioGroup defaultValue="general" value={generationType} onValueChange={(value: GenerationType) => setGenerationType(value)} className="grid grid-cols-3 gap-3">
                  {(['general', '6-digit', '8-digit'] as const).map(value => (
                      <div key={value}>
                          ...
                          <Label htmlFor={value} className="flex h-full cursor-pointer flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-4 text-sm font-medium hover:bg-background peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary capitalize">
                              {passwordTypes[value]}
                          </Label>
                      </div>
                  ))}
